...
$query_Recordset1 = sprintf("SELECT * FROM administrators WHERE id = %s", GetSQLValueString($colname_Recordset1, "int"));
...
if ($row_loggedin['power']==100) {
	
	
if ((isset($_GET['id'])) && ($_GET['id'] != "")) {
  $deleteSQL = sprintf("DELETE FROM administrators WHERE id=%s",
                       GetSQLValueString($_GET['id'], "int"));

  mysql_select_db($database_ststsconfig, $ststsconfig);
  $Result1 = mysql_query($deleteSQL, $ststsconfig) or die(mysql_error());

  $deleteGoTo = "server-admins.php";
