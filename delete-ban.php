...
mysql_select_db($database_ststsconfig, $ststsconfig);
$query_Recordset1 = sprintf("SELECT * FROM banned_players WHERE id = %s", GetSQLValueString($colname_Recordset1, "int"));
$Recordset1 = mysql_query($query_Recordset1, $ststsconfig) or die(mysql_error());
...
if ((isset($_GET['id'])) && ($_GET['id'] != "")) {
	...
	$odabran = $row_Recordset1['screenshot_url'];
   
   if (file_exists($odabran)) {
    unlink($odabran);
   }
  ...
  $deleteSQL = sprintf("DELETE FROM banned_players WHERE id=%s",
                       GetSQLValueString($_GET['id'], "int"));

  mysql_select_db($database_ststsconfig, $ststsconfig);
  $Result1 = mysql_query($deleteSQL, $ststsconfig) or die(mysql_error());
